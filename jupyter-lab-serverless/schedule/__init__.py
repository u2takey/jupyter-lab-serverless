...
def remove_job(name):
    ...
    :data:`default scheduler instance <default_scheduler>`.
    """
    if name in default_scheduler.name_to_jobs:
        cancel_job(default_scheduler.name_to_jobs[name])
        del default_scheduler.name_to_jobs[name]


def add_job(name, spec, job, *args, **kwargs):
    """
    :param spec: valid spec {'every': 1, unit: 'day', at: '8:00'}
    :param name: job Name
    :param job: job
    :param args: args
    :param kwargs: kwargs
    :return: Job
    """
    every_unit = spec.get('every', 1)
    if not isinstance(every_unit, int):
        every_unit = 1
    unit = spec.get('unit', 'day')
    at = spec.get("at", None)
    d = default_scheduler.every(every_unit)
    try:
        d = getattr(d, unit)
    except Exception as e:
        logging.info('unit not valid {e}'.format(e=e))
    ...
    default_scheduler.name_to_jobs[name] = d
    return d.do(job, *args, **kwargs)
